# Data Accessor - проміжний "шар" (layer) між контролерами та даними (БД)
import json
...
class DataAccessor:
    def __init__(self, ini_file:str='db_ini.json') :
        try :
            with open(ini_file, mode="r", encoding='utf-8') as file :
    ...
    def _seed_token(self) :
        ...
        token_id = str(uuid.uuid4())                 # випадковий UUID токена
        ua_id = '35326873-9852-11f0-b1b7-62517600596c'  # admin user_access id
        issued_at = datetime.now()              
        expired_at = issued_at + timedelta(days=1)  

        sql = """INSERT INTO tokens(token_id, ua_id, token_issued_at, token_expired_at)
        VALUES(?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE 
            ua_id = VALUES(ua_id),
...
def main() :
    ...
    if dataAccessor.install(with_seed=True) :
        print("Install OK")
    else :
        return
    print(helper.salt())
